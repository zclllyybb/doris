...
Always use only the `build.sh` script with its correct parameters to build Doris BE and FE. When building, use at least `-j60` parallelism. For example, the simplest BE+FE build command is `./build.sh --be --fe -j60`.
...
BE-UT compilation must not be below 60 parallelism.
