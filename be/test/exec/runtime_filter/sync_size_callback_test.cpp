...
// =====================================================================
// DummyBrpcCallback error handling tests
// =====================================================================

class DummyBrpcCallbackTest : public RuntimeFilterTest {
protected:
...
TEST_F(DummyBrpcCallbackTest, rpc_fail_cancels_query_when_context_provided) {
    auto ctx = make_query_ctx();
    auto callback = DummyBrpcCallback<PMergeFilterResponse>::create_shared(ctx);
    callback->cntl_->SetFailed("injected failure");
...
TEST_F(DummyBrpcCallbackTest, rpc_fail_no_cancel_when_no_context) {
    // Simulates ignore_runtime_filter_error=true pattern: pass empty weak_ptr
    auto callback = DummyBrpcCallback<PMergeFilterResponse>::create_shared();
    callback->cntl_->SetFailed("injected failure");
...
TEST_F(DummyBrpcCallbackTest, response_error_cancels_query_when_context_provided) {
    auto ctx = make_query_ctx();
    auto callback = DummyBrpcCallback<PMergeFilterResponse>::create_shared(ctx);
    auto* status_pb = callback->response_->mutable_status();
    status_pb->set_status_code(TStatusCode::INTERNAL_ERROR);
...
TEST_F(DummyBrpcCallbackTest, response_error_no_cancel_when_no_context) {
    auto callback = DummyBrpcCallback<PMergeFilterResponse>::create_shared();
    auto* status_pb = callback->response_->mutable_status();
    status_pb->set_status_code(TStatusCode::INTERNAL_ERROR);
    status_pb->add_error_msgs("injected error");
...
TEST_F(DummyBrpcCallbackTest, success_path_no_cancel) {
    auto ctx = make_query_ctx();
    auto callback = DummyBrpcCallback<PMergeFilterResponse>::create_shared(ctx);
...
TEST_F(DummyBrpcCallbackTest, expired_query_context_no_crash) {
    std::weak_ptr<QueryContext> expired_ctx;
    {
    ...
    auto callback = DummyBrpcCallback<PMergeFilterResponse>::create_shared(expired_ctx);
    callback->cntl_->SetFailed("injected failure");
...
TEST_F(DummyBrpcCallbackTest, end_of_file_status_does_not_cancel) {
    auto ctx = make_query_ctx();
    auto callback = DummyBrpcCallback<PMergeFilterResponse>::create_shared(ctx);
    auto* status_pb = callback->response_->mutable_status();
