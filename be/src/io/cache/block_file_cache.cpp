...
size_t BlockFileCache::repair_duplicate_ttl_dirs_once() {
    ...
    {
        SCOPED_RAW_TIMER(&duration_ns);
        auto st = _storage->list_duplicate_key_dirs(&duplicate_dirs);
        if (!st.ok()) {
    ...
    *_ttl_repair_checker_suspect_hashes << duplicate_dirs.size();
    ...
    const int64_t qps_limit = config::file_cache_ttl_repair_checker_qps_limit;
    if (max_repairs == 0 || qps_limit <= 0) {
        *_ttl_repair_checker_latency_us << (duration_ns / 1000);
        return 0;
    }

    const auto sleep_per_repair =
            std::chrono::microseconds(std::max<int64_t>(1, 1000000 / qps_limit));
